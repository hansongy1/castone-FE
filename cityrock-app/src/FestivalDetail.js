...
const festivalDetails = [
  {
    ...
    introduce: '2024 차 없는 잠수교 뚜벅뚜벅 축제는 오감으로 만나는 힐링 놀이터의 컨셉으로 개최한다. 매주 일요일, 몸과 마음을 쉬어갈 수 있는 한강 위의 축제 놀이와 힐링을 동시에 즐기며 컬러풀한 한강으로 충전 할 수 있는 축제이다. 매주 특별한 프로그램과 다양한 공연, 힐링존, 푸드트럭 등을 준비하고, 전국의 농부들을 만나볼 수 있는 농부의 시장, 플리마켓, 놀이 체험 공간 등 다양한 즐길 거리를 구성하여 참여하는 행사 방문객들에게 즐거움과 휴식을 동시에 제공하고자 한다.',
  },
  {
    id: 2,
    title: '곡성세계장미축제',
...
const FestivalDetail = () => {
  const { id } = useParams();
  const festival = festivalDetails.find((festival) => festival.id === parseInt(id));
  ...
  return (
    <div className="festival-detail">
      ...
      {festival.introduce && <p><h4>소개</h4>{festival.introduce}</p>}
    </div>
  );
};
